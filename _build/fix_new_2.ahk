...
LastMetaFile := ""
ffmpeg := A_ScriptDir . "\rpcl3_tools\ffmpeg.exe"  ; Fixed path
player := ""
MyGuiHwnd := ""
isPlaying := false
isPaused := false
...
CheckDependencies:
    ...
    if !FileExist(ffmpeg) {
        MsgBox, 48, Warning, ffmpeg.exe not found in /rpcl3_tools folder.`nMP3 conversion will not be available.
...
CreateGUI:
    ; Main ListView - removed duplicate vTrackList
    Gui, Add, ListView, gTrackClicked x0 y0 w515 h227 AltSubmit Grid, Name|Size|Bitrate|Type|Duration
    ...
    if (row) {
        ...
        if !ErrorLevel {
            ...
            if !ErrorLevel {
                ...
                Log("ERROR", "File rename failed: " . filePath . " -> " . newPath)
            }
        }
    }
return
    ...
    if (row) {
        ...
        IfMsgBox Yes
        {
            ...
            if !ErrorLevel {
                ...
                Log("ERROR", "File deletion failed: " . filePath)
            }
        }
    }
return
...
ConvertToMP3:
    if !FileExist(ffmpeg) {
        MsgBox, 16, Error, ffmpeg.exe not found in /rpcl3_tools folder.
        return
    }
